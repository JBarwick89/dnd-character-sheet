...
export default function CharacterCreator() {
  ...
  const saveCharacter = async() => {
    if (character.name && character.playerName) {
      ...
      await axios.post('http://localhost:8000/character', character)
      .then(res => {
        setLoading(false);
  ...
  return (
    <div className="characterCreator">
      ...
      {
        showAlert &&
        <Alert variant="filled" color="red" radius="xl" withCloseButton icon={<IconInfoCircle />} onClose={() => setShowAlert(false)}>
            Fill out the required fields.
        </Alert>
      }

      <SheetHeader onEnterKey={saveCharacter} />
      <Button onClick={saveCharacter} loading={loading} loaderProps={{type: 'dots'}}>
        Save
      </Button>
